...
const contractAddress = import.meta.env.VITE_CIPHER_AGENT_PAY_ADDRESS ?? "";
  ...
  return (
    <main className="app-shell">
      <header className="hero">
        <p className="eyebrow">Confidential Onchain Finance · Zama FHEVM</p>
        <h1>Pay like a private CFO.</h1>
        <p className="lede">
          CipherAgent Pay gives autonomous agents an encrypted spending wallet on Sepolia.
          Set the policy. Let the agent transact. Reveal only what your role is allowed to see.
        </p>
        <div className="hero-actions">
          <button onClick={connectWallet} disabled={isBusy}>
            {account ? shortAddress(account) : "Connect Sepolia wallet"}
          </button>
          <span className="network-pill">Sepolia · Zama Relayer</span>
          <span className="network-pill subtle">
            {isConfigured ? `Contract ${shortAddress(contractAddress)}` : "Contract not configured"}
          </span>
        </div>
      </header>
      ...
      <section className="stages">
        <article className="stage">
          <div className="stage-aside">
            ...
            <span className="stage-tag">Owner</span>
          </div>
          <div className="stage-body">
            <h2>Encrypt the spending policy.</h2>
            <p>
              Budget, per-payment cap, and total spend cap leave the browser as FHE ciphertext.
              Agent and an optional auditor are bound to the policy at creation.
            </p>
            <div className="form-grid">
              ...
              <label>
                Initial merchant
                <input value={merchantAddress} onChange={(event) => setMerchantAddress(event.target.value)} placeholder="0x..." />
              </label>
              <label>
                Auditor (optional)
                <input value={auditorAddress} onChange={(event) => setAuditorAddress(event.target.value)} placeholder="0x..." />
              </label>
              <label>
                Initial budget
              ...
              <label>
                Per-payment limit
                <input value={perPaymentLimit} onChange={(event) => setPerPaymentLimit(event.target.value)} />
              </label>
              <label>
                Total spend limit
                <input value={totalSpendLimit} onChange={(event) => setTotalSpendLimit(event.target.value)} />
              </label>
            </div>
            <div className="stage-actions">
              <button onClick={createEncryptedAgent} disabled={isBusy}>
                {agentState?.initialized ? "Rotate encrypted policy" : "Encrypt policy"}
              </button>
              {agentState?.initialized && (
                <button className="ghost" onClick={togglePause} disabled={isBusy}>
                  {agentState.paused ? "Resume agent" : "Pause agent"}
                </button>
              )}
            </div>
            {agentState?.initialized && (
              <p className={`stage-footnote ${agentState.paused ? "warn" : ""}`}>
                {agentState.paused ? "Policy paused" : "Policy live"}
                {" · agent "}{shortAddress(agentState.agent)}
                {" · auditor "}{shortAddress(agentState.auditor)}
                {merchantAllowed ? " · merchant approved" : ""}
              </p>
            )}
          </div>
        </article>
        ...
        <article className="stage">
          <div className="stage-aside">
            ...
            <span className="stage-tag">Agent</span>
          </div>
          <div className="stage-body">
            <h2>Spend without revealing.</h2>
            <p>
              The amount is encrypted in the browser. The contract checks balance and limits on
              ciphertext and updates state in the same transaction.
            </p>
            <div className="form-grid">
              <label>
                Owner
            ...
            <button onClick={requestPayment} disabled={isBusy}>Submit encrypted payment</button>
          </div>
        </article>
        ...
        <article className="stage">
          <div className="stage-aside">
            ...
            <span className="stage-tag">Disclosure</span>
          </div>
          <div className="stage-body">
            <h2>Decrypt only what your role allows.</h2>
            <p>
              The connected wallet signs an EIP-712 request. The Zama relayer returns plaintext
              for handles your address has been granted, and nothing else.
            </p>
            <button onClick={decryptMyView} disabled={isBusy}>Decrypt my view</button>
            {Object.keys(decryptResults).length > 0 ? (
              ...
              <p className="stage-footnote subtle">
                Owner and auditor decrypt balance, spend, and last payment status. Merchant decrypts
                only its own revenue.
              </p>
            )}
          </div>
        </article>
